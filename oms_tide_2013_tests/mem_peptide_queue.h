...
//#include "sp_scorer.h"
#ifndef MY_ACTIVE_PEPTIDE_QUEUE_H
#define MY_ACTIVE_PEPTIDE_QUEUE_H

class TheoreticalPeakCompiler;

class ActivePeptideQueue2 {
public:
	ActivePeptideQueue2(InMemIndex* index, RecordReader* reader, const vector<const pb::Protein*>& proteins);
...
//#include "sp_scorer.h"
#ifndef MEM_PEPTIDE_QUEUE_H
#define MEM_PEPTIDE_QUEUE_H

class MemPeptideQueue : ActivePeptideQueue
{

public:
	MemPeptideQueue(InMemIndex* index, RecordReader* reader, const vector<const pb::Protein*>& proteins);
	int MemSetActiveRange(vector<double>* min_mass, vector<double>* max_mass, double min_range, double max_range, vector<bool>* candidatePeptideStatus);
...
MemPeptideQueue::MemPeptideQueue(InMemIndex* index, RecordReader* reader, const vector<const pb::Protein*>& proteins) : ActivePeptideQueue(reader, proteins)
{
	index_ = index;
}


int MemPeptideQueue::MemSetActiveRange(vector<double>* min_mass, vector<double>* max_mass, double min_range, double max_range, vector<bool>* candidatePeptideStatus)
{
	while (!this->queue_.empty() && queue_.front()->Mass() < min_range) {
